...
Answer: 20,332,093. We can see the total number of records in the Details tab of the materialized table.
...
reading more data than querying one column (PULocationID), leading to a higher estimated number of bytes processed. 155.12 MB for the first query and 310.24 MB (the double) for the second query
...
BigQuery will process 0MB since the number of rows is stored in the metadata of the table
